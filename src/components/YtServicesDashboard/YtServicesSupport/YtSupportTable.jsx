...
import useSupportTableDataQuery from "@/hooks/Dashboard/useSupportTableDataQuery";
...
export default function YtSupportTable() {
  const supportData = useSupportTableDataQuery();
        ...
        return (
          <p
            className={`py-1 px-6 rounded-full text-sm ${
              status.toLowerCase() === "resolved" ? active : pending
            }`}
          >
            {status}
          </p>
  ...
  return (
    <div className="bg-white w-full p-8 rounded-2xl">
      <p className="text-2xl font-bold mb-4">My Tickets</p>
